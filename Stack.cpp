...
public:
    ...
    T Pop()  {
        ...
        while (iter->next != tail){ //tailden önceki kutuya ilerle
            iter = iter->next;
        }
        temp = tail->next; // tailden sonraki kutuyu temp'e al
        tempdata = tail->next->x; //datayı al
        tail = iter; //kuyruğu kaydır
        tail->next->next = NULL; // atamayı yap
